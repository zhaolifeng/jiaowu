...
import com.shenlan.common.utils.Reflections;
...
import com.shenlan.domain.ExcelField;
		...
		for (int i = this.getDataRowNum(); i < this.getLastDataRowNum(); i++)
		{
			...
			for (Object[] os : annotationList)
			{
				...
				if (val != null)
				{
					...
					{
//						val = ScmTypeDicStorage.getInstance().findTypeByTypeName(ef.dictType(), val == null ? "" : val.toString());
						val = "字典类型对应的值"; 
					}
					...
					else if (os[1] instanceof Method)
					{
						...
						Reflections.invokeMethod(e, mthodName, new Class[]
						{ valType }, new Object[]
						{ val });
					}
				}
	...
	public static void main(String[] args) throws Throwable
	{
		...
		ImportExcel ei = new ImportExcel("d://a.xlsx", 1);
		...
		for (int i = ei.getDataRowNum(); i < ei.getLastDataRowNum(); i++)
		{
			...
			if (row != null)
			{
				for (Cell c : row)
				{
					boolean isMerge = ei.isMergedRegion(i, c.getColumnIndex());
					Object val = null;
					if (isMerge)
					{
						val = ei.getMergedRegionValue(i, c.getColumnIndex());
					}
					else
					{
						val = ei.getCellValue(c);
					}
